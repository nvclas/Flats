...
public class FlatsCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!command.getName().equalsIgnoreCase("flat")) return true;
        ...
        switch (subCommand) {
            case "select" -> {
                if (!Flats.checkPermission(p, "flats.admin")) return true;
            ...
            case "add" -> {
                ...
                for (String flat : flatsConfig.getConfig().getKeys(false)) {
                    for (String selectionString : flatsConfig.getAreas(flat)) {
                        Selection selection = flatsConfig.getSelectionFromString(selectionString);
